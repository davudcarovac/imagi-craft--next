...
export async function signupUser(req, res, next) {
    ...
    try {
        ...
        if (existingUser)
            throw new ErrorResponse("User already registred", 400);
...
export async function changePassword(req, res, next) {
    ...
    try {
        ...
        if (isSamePassword) {
            throw new ErrorResponse("Nova lozinka ne može biti ista kao trenutna", 400);
        }
        const salt = await bcrypt.genSalt(10);
        const hashedNewPassword = await bcrypt.hash(newPassword, salt);
        await prisma.user.update({
...
async function removeImage(userId) {
    try {
        ...
        if (user?.profileImagePublicId) {
            const result = await cloudinary.uploader.destroy(user?.profileImagePublicId);
            if (result.result === "ok") {
                await prisma.user.update({
                    where: { id: userId },
                    data: {
...
export async function removeProfileImg(req, res, next) {
    try {
...
export async function uploadProfileImg(req, res, next) {
    try {
        const userId = req.userData.id;
        const user = await prisma.user.findUnique({
            where: { id: userId },
...
export async function setupTwoFactor(req, res, next) {
    ...
    try {
        ...
        QRCode.toDataURL(secret.otpauth_url, (err, dataUrl) => {
            if (err)
                throw new ErrorResponse("Qr code error", 500);
            res.json({
                success: true,
...
export async function verifyEnableTwoFactor(req, res, next) {
    const { id } = req.userData;
...
export async function disableTwoFactor(req, res, next) {
    ...
    try {
        ...
        if (!user.twoFactorEnabled) {
            throw new ErrorResponse("Two-Factor Authentication is already disabled", 400);
        }
        await prisma.user.update({
            where: { id: id },
            data: {
...
export async function removeTokens(req, res, next) {
